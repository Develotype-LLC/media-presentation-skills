/* ============================================================================
   ...
   USAGE
     mountScrollWorld(document.getElementById('world'), {
       ...
       architecture: 'dive-connect', // or 'continuous'
       strict: true,      // fail visibly when required media/config is missing
       fps: 16,           // endpoint seek stops on the final decodable frame
       fit: 'cover',      // use 'contain' for simultaneous landscape comparisons
       ...
       connectors: [clipUrl, …],          // dive-connect: exactly sections.length - 1
       // connector objects may provide their own exact first-frame poster:
       // { clip:'...', clipMobile:'...', poster:'...', posterMobile:'...' }
...
function mountScrollWorld(container, config) {
  ...
  const ARCHITECTURE = config.architecture || (CONNECTORS.length ? 'dive-connect' : 'continuous');
  const STRICT = config.strict !== false;
  const FPS = config.fps || 16;
  const N = SECTIONS.length;
  if (!N) return;

  if (!['continuous', 'dive-connect'].includes(ARCHITECTURE)) {
    throw new Error(`scroll-world: unknown architecture "${ARCHITECTURE}"`);
  }
  if (STRICT && SECTIONS.some((s, i) => !s || !s.clip)) {
    throw new Error('scroll-world: strict mode requires one clip for every section');
  }
  if (STRICT && ARCHITECTURE === 'continuous' && CONNECTORS.filter(Boolean).length) {
    throw new Error('scroll-world: continuous architecture must use connectors: []');
  }
  if (STRICT && ARCHITECTURE === 'dive-connect' &&
      (CONNECTORS.length !== N - 1 || CONNECTORS.some(c => !c))) {
    throw new Error(`scroll-world: dive-connect requires exactly ${N - 1} connectors`);
  }

  injectCSS();
  container.classList.add('sw-root');
  container.style.setProperty('--sw-fit', config.fit || 'cover');
  ...
  SECTIONS.forEach((s, i) => {
    ...
    SEGMENTS.push(dive);
    s._seg = dive;
    if (ARCHITECTURE === 'dive-connect' && i < N - 1 && CONNECTORS[i]) {
      const connector = typeof CONNECTORS[i] === 'string'
        ? { clip: CONNECTORS[i], clipMobile: CONNECTORS_M[i] }
        : CONNECTORS[i];
      SEGMENTS.push({
        kind: 'conn', si: i, clip: connector.clip,
        clipM: connector.clipMobile || CONNECTORS_M[i],
        still: connector.poster, stillM: connector.posterMobile,
        accent: SECTIONS[i + 1].accent, w: connector.scroll || CONN_W
      });
    }
  });
  ...
  function loadClip(s) {
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        ...
        s.loading = false;
        s.error = error;
        if (STRICT) showError(`Required camera segment failed to load: ${s.clip}`);
      });
  }
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      let op = 0;
      if (y >= s.start && y <= s.end) op = 1;
      else if (y < s.start && y >= s.start - fade) {
        op = smooth(1 - (s.start - y) / fade);
      } else if (y > s.end && y <= s.end + fade) {
        op = 1 - smooth((y - s.end) / fade);
      }
      s.el.style.opacity = op; s.visible = op > 0.001;
      // Later segments sit above earlier ones, so their pre-boundary opacity creates
      // a genuine incoming crossfade instead of a hard z-index switch.
    ...
    hint.style.opacity = SEGMENTS[0].ready ? clamp(1 - y / (0.5 * vh)) : 0;
  ...
  function raf() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      const lastFrameTime = Math.max(0, dur - 1 / FPS);
      const t = clamp(s.cur, 0, 1) * lastFrameTime;
  ...
  function showError(message) {
    let box = container.querySelector('.sw-error');
    if (!box) {
      box = el('div', 'sw-error');
...
function injectCSS() {
  ...
  .sw-scene__video,.sw-scene__still{position:absolute;inset:0;width:100%;height:100%;object-fit:var(--sw-fit,cover);object-position:center 42%;}
  ...
  .sw-error{position:fixed;z-index:100;right:18px;bottom:18px;max-width:min(520px,calc(100vw - 36px));padding:12px 16px;border:1px solid #ff9f9f;border-radius:10px;background:#2a0808;color:#fff;font:600 13px/1.45 ui-monospace,Menlo,monospace;box-shadow:0 12px 40px #0006;}
